...
<body data-theme="light">
  ...
  <script>
    ...
    function updateWorldTransform() {
      world.style.transform = "translate3d(" + state.offset.x + "px," + state.offset.y + "px,0)";
      panStatus.textContent = "偏移 " + Math.round(state.offset.x) + ", " + Math.round(state.offset.y);
    }
    ...
    function renderConnections() {
      ...
      for (const sourceMap of MAP_DATA) {
        const sourcePosition = state.placed.get(sourceMap.id);
        if (!sourcePosition) continue;
        for (const exit of sourceMap.exits) {
          const targetPosition = state.placed.get(exit.targetMapId);
          if (!targetPosition) continue;

          const x1 = sourcePosition.x + (exit.x + 0.5) * CELL;
          const y1 = sourcePosition.y + (exit.y + 0.5) * CELL;
          const x2 = targetPosition.x + (exit.targetX + 0.5) * CELL;
          const y2 = targetPosition.y + (exit.targetY + 0.5) * CELL;
    ...
    function stagePositionFromClient(clientX, clientY, map) {
      ...
      return {
        x: clientX - rect.left - state.offset.x - (map.w * CELL) / 2,
        y: clientY - rect.top - state.offset.y - (map.h * CELL) / 2
      };
    }
    ...
    function placeAtCenter(mapId) {
      ...
      const rect = stage.getBoundingClientRect();
      placeMap(mapId, {
        x: rect.width / 2 - state.offset.x - (map.w * CELL) / 2,
        y: rect.height / 2 - state.offset.y - (map.h * CELL) / 2
      });
    }
    ...
    function handlePointerMove(event) {
      ...
      if (drag.kind === "card") {
        ...
        if (position) {
          position.x = drag.startPosition.x + deltaX;
          position.y = drag.startPosition.y + deltaY;
    ...
    function validateLayout(payload) {
      ...
      return { offset, placed, unknownCount, duplicateCount, invalidCount };
    }
