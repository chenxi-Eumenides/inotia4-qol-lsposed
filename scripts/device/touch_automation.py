...
uv run python scripts/device/touch_automation.py click 100,200 0.5 down 300,400 0.2 up 300,400 0.1
uv run python scripts/device/touch_automation.py wait 2.0 click 500,500 0.3

  # 检测模式：不传参数，实时打印屏幕上的每个触摸动作（时刻/位置/间隔）
uv run python scripts/device/touch_automation.py
...
坐标空间：输入与检测输出的坐标统一为当前方向的应用窗口坐标（按设备实际分辨率），
...
def require_device() -> None:
    r = adb("devices")
    lines = [line for line in r.stdout.splitlines()[1:] if line.strip() and "device" in line]
    if not lines:
