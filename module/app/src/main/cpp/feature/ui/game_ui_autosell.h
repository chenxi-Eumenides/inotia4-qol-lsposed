...
// 自动出售 UI（auto-sell UI）：背包页齿轮入口按钮 + 按存档配置面板。
//
...
//   UI_SetPopupProcessInfo(1, id) 打开、(3, 0) 关闭；面板自绘并编辑配置草稿。
//
...
//
// 关闭按钮释放时一次性调用 autosell_apply_config + autosell_store_persist；编辑期间不落盘。
