...
- `scripts/build-release.sh` 仅在用户明确要求 release 时使用；它执行 `assembleRelease`，读取 `versionName`，并将当前未配置签名的 Release APK 复制为 `output/inotia4-qol-lsposed-v<version>-release-unsigned.apk`。
- Debug 构建会将 APK 复制为 `output/inotia4-qol-lsposed-debug-<YYMMDDHHMM>-<sha256前12位>.apk`，并只保留最新 3 份 Debug APK；Release 构建只保留最新 2 份 Release APK。
