...
- `scripts/build-release.sh` 仅在用户明确要求 release 时使用；它执行 `assembleRelease`，读取 `versionName`，把 Gradle 未签名的 Release APK 经 zipalign + apksigner 用默认 debug keystore（`~/.android/debug.keystore`，`CN=Android Debug`）签名后输出为 `output/inotia4_qol_lsposed_release_v<version>.apk`。
