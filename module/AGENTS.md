...
- `scripts/build-release.sh` 仅在用户明确要求 release 时使用；它执行 `assembleRelease`，读取 `versionName`，并将当前未配置签名的 Release APK 复制为 `output/inotia4_qol_lsposed_release_unsigned_v<version>.apk`。
