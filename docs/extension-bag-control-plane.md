...
| Current gate | P1 逻辑背包模型与 sidecar 原型（验收路径已就绪：原版背包 API 并入扩展袋 + `/api/debug/extension_bag/*` 视图控制） |
| Blocking issue | 无阶段级阻断；P1 完成要求（容量派生逆向、所有权状态机、prepare journal、host 测试）待实施 |
| Next allowed action | 启动 P1：模型设计与 host 测试优先；真机数据/移动验收使用 `/api/item/inventory*`（bag 6..10），视图控制使用 `/api/debug/extension_bag/*` |
...
| P3 | 🔄 可启动（E-2026-08-29-01：投影就绪，所有权账本待接线） | 原版控件与扩展窗口的正式接入 |
