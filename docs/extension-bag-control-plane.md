...
>
> **文档版本**：v1.16 ｜ **状态**：CURRENT ｜ **最后修改**：2026-08-30 ｜ **最近审核**：2026-08-30
> **当前阶段**：P0/P1/P2 已通过（对应历史证据见 §5.2、§9 与 §13）；P3 标签挂载稳定性已通过、其余原版控件语义仍在验收 ｜ **当前闸门**：P3 信息页、选中反馈与输入行为的单项验证

## 0. 控制面恢复块

- **唯一入口**：会话压缩、换代理或重新打开任务时，先读本节，再读“当前状态”和“当前闸门”，不得以聊天记录或旧日志替代事实。
- **当前结论**：`NOT_ACCEPTED`。P0/P1/P2 的历史通过不替代 P3 及最终真机验收；源码存在、host 测试或单次 API 调用均不能视为 P3 功能完成。
- **下一允许动作**：一次只验证一个控件行为；扩展袋标签挂载动画期稳定性已由 `E-2026-08-30-01` 通过，下一项是二次点击扩展标签时的原版信息页与解除语义。
...
- P0/P1/P2 已有历史通过记录；P3 控件树、投影和输入接入代码存在，标签挂载动画期稳定性已通过，但其余单项真机验收未完成。
...
| Current phase | `P3` 原版控件与扩展窗口正式接入（实现中，未通过） |
| Current gate | P3 控件行为与 UI 完整性；数据/移动正式操作面仍遵循 ADR-008 的原版背包 API |
| Blocking issue | P3 原版信息页/解除语义、按下与唯一选中反馈、拖动输入尚无本阶段真机证据；不得将标签稳定性或 P2 投影证据外推为 P3 通过 |
| Next allowed action | 固化当前构建/工作树身份后，真机只验证二次点击扩展标签的原版信息页与解除语义；结果登记新证据 ID |
| Owner | 当前执行代理；同一验收项一次只允许一个代码、构建或设备变量 |
| Evidence | P0 `E-2026-08-28-01`；P1 `E-2026-08-28-03`（引用块待补）；P2 `E-2026-08-29-01`（引用块待补）；P3 标签稳定性 `E-2026-08-30-01` |
...
| P3 | 🔄 进行中（`E-2026-08-30-01` 仅通过标签挂载稳定性；其余控件语义未验收） | 原版控件与扩展窗口的正式接入 |
...
**P3 当前实现与未验收边界（v1.15 对账）**

- **Confirmed facts**：扩展标签已改为 `ControlItem`，以真实袋物品对象作为 `data[0]`，挂载在原版袋容器并用父链绝对位置换算；容器子控件不足 6 时延迟重试，避免面板初期开到占位容器。`E-2026-08-30-01` 已由用户真机手动确认面板开合动画期间标签无移动或偏移。当前代码还实现了同一标签二次点击设置 `desc_type=1` 并调用原版 `UIEquip_MakeDesc`。
- **Not completed**：原版信息页/解除语义、按下与唯一选中反馈均无 P3 真机证据；`drawdiag` 诊断日志尚未移除。G-8 的 drop 路由已存在，但 `0x81` 拖动建立协议仍被用户挂起；G-9 仅完成扩展标签目标的 `GetItemSlotIndex >= 16` 防护，`GetBagSlotIndex` 语义仍待定。
- **Do not repeat**：不得按旧 handoff 重启 depth1 挂载试验，也不得以代码注释“天然不漂移”、P2 压测或 host 测试宣称 P3 已通过。
- **Next exact action**：记录当前源码/构建/设备身份后，唯一验证同一扩展标签二次点击是否打开对应袋的原版信息页，并核对解除按钮语义；通过或失败均登记新的 P3 证据 ID，再进入选中效果验收。
- **Required evidence**：使用 §5.1 模板，包含当前逻辑袋、索引 `5` sentinel、信息页控件树/描述状态、日志锚点和结果；不改变其他 P3 变量。
