...
>
> **文档版本**：v1.17 ｜ **状态**：CURRENT ｜ **最后修改**：2026-08-31 ｜ **最近审核**：2026-08-31
> **当前阶段**：P0/P1/P2 已通过（对应历史证据见 §5.2、§9 与 §13）；P3 标签稳定性、扩展袋装备/解除、选中反馈已由用户真机确认 ｜ **当前闸门**：P3 收尾（全满弹窗 6 边界确认）；扩展拖动建立协议移出本阶段、转后续
...
- **下一允许动作**：一次只验证一个控件行为；扩展袋标签挂载动画期稳定性已由 `E-2026-08-30-01` 通过；扩展袋装备（拖放/装备按钮两入口）与解除已由用户真机确认（`E-2026-08-31-01`），选中反馈确认无问题（`E-2026-08-31-02`）；下一项是装备时原版+扩展全满的弹窗 6 边界（透传路径，交用户点击确认）。
...
| Evidence | P0 `E-2026-08-28-01`；P1 `E-2026-08-28-03`（引用块待补）；P2 `E-2026-08-29-01`（引用块待补）；P3 标签稳定性 `E-2026-08-30-01`；P3 装备/解除 `E-2026-08-31-01`；P3 选中反馈 `E-2026-08-31-02` |
...
| P3 | 🔄 收尾中（标签稳定 `E-2026-08-30-01`、装备/解除 `E-2026-08-31-01`、选中反馈 `E-2026-08-31-02`；全满弹窗 6 待用户点击确认；扩展拖动转后续） | 原版控件与扩展窗口的正式接入 |
...
- **Not completed**：装备时原版+扩展**全满**的弹窗 6 边界仅代码保证（扩展无空位时透传原 `UIEquip_ButtonEquipExe`），待用户点击确认；`0x81` 扩展拖动建立协议由用户明确转后续（原版拖动可用）；G-9 `GetBagSlotIndex` 语义待定。原版与扩展物品数据不一致属预期——物品真实所有权流动（P4）尚未开始。
