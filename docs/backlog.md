...
|---|---|---|---|---|---|
| 探索中 | **存档完整性检测与安全进档** | 2026-08-27，唯一真机 `192.168.3.54` 的已有存档 0 在游戏中显示“存档损坏”；调用 `POST /api/system/enter_slot` 无法进入并导致游戏崩溃。当前 API 没有进入前预检、错误分类或阻断机制，不能把该存档继续用于扩展背包验收。损坏原因尚未确认，不得归因于模块、原版游戏或具体文件，直到取得原版/模块日志和存档字节证据。 | ① 逆向并记录游戏原版检查链：存档文件枚举、大小/魔数/版本/槽位/generation/CRC 或校验和、解密/解压、记录边界、角色/背包解析、失败返回和 UI“存档损坏”判定；② 定义只读 `save preflight` 能力，进入前返回 `valid / corrupt / missing / incompatible / unknown`、失败阶段、可诊断原因和是否允许进入；③ 改造 `enter_slot`：先完成预检，非 `valid` 时只返回结构化错误，禁止调用会触发崩溃的原版进档路径；④ 对原版保存、sidecar 提交、进程中断和异常退出建立备份/原子替换/last-good/恢复策略；⑤ 对当前存档 0 获取原始文件、模块 sidecar、版本/签名/日志，定位究竟是哪一层无法解析及损坏发生时点；⑥ 增加 host 样本测试和真机故障注入，验证预检不会修改存档且拒绝进档不会崩溃。 | 对存在、损坏、缺失、版本不兼容、校验失败、截断、空槽和解析异常样本，API 均在调用原版进档前返回明确结构化结果；不可进入时游戏进程不崩溃、不改变原版存档；可进入样本成功进入；保存/sidecar 失败保留可恢复副本和诊断证据；根因与预防措施有字节、日志、调用链证据。未完成真机验证前保持“探索中”。 | 用户 2026-08-27 真机 2 反馈 |
