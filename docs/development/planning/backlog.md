...
| 已实现，真机验证通过 | **强化（enchant）扩展物品接管** | 同袋扩展强化卷轴→扩展装备进入 apply branch，经 `UIEquip_IsApplyStuff@0xb8d4c`、`SAVE_IsOK@0x128c14` 和 `UIEquip_ApplyStuff@0xb8df8` 调原版强化链，`EnchantItem`/`ConsumeItem` 完成强化与消费；失败 Blocked 不降级 swap | 已执行 VM-10/S-03；确认强化卷轴只消费一次、装备 payload/等级同步、成功路径无 extension swap | `verification-matrix.md` S-03、VM-10；提交 `19dbc77` |
| 已实现，真机验证通过 | **扩展 apply 材料拖装备的函数级接管** | 第 15 个常驻 Native Hook 接管 `UIEquip_EquipControlEventProc@0xb8f7c`；event=0x04、扩展宝石/强化卷轴源且目标为装备槽时放行原版 apply 链，失败 Blocked 不降级；同袋 apply 分支经 `UIEquip_ApplyStuff@0xb8df8` 路由 | 已执行 VM-10/VM-B03；确认宝石/卷轴各只消费一次且成功路径不发生 extension swap | `verification-matrix.md` S-03、VM-10、VM-B03；提交 `19dbc77` |
...
| 探索中 | **NetworkStore_* 两写点勘察（S2 写侧）** | `native_inventory_hook.cpp` 语义表将 `NetworkStore_InitializeMenuData@0x15b7b0`、`NetworkStore_AddItem@0x15d640` 列为「待勘察」：数量来自网络数据 `+0x18`，离线不可达无法冻结值域；权威路由：[`control-plane.md`](../features/extension-bag/control-plane.md) §2 | 联网抓包/内存 dump 商品表与收货数据 `+0x18` 域，或真机挂回调记录实际数量域；冻结后判定是否需要 S2 写侧回写 | `NetworkStore_InitializeMenuData`/`NetworkStore_AddItem` 数量域有真机证据，结论登记进 `rulebook.md` R-49 与 `runtime-architecture.md` §2.1.1 | 本轮语义表反汇编冻结 |
| 待定 | **S2 原版写点 >127 进位闭合** | 原版 `SetBitValue(start=25,end=31)` 只能写 b 段；合并/创建总量 >127 时无法对 a 进位（≤127 全部正确），当前由调用方级 H-18..H-21 闭合，仍未覆盖全部原版写点；权威路由：[`control-plane.md`](../features/extension-bag/control-plane.md) §2 | 逐写点核对是否存在把 S2 全量（>127）作 count 的路径（如丢弃/移仓、静态表 >99 产物量），必要时补调用方级写点或数据审计 | 全部数量写点在 >127 场景有结论；开放边界清零或明确登记 | `runtime-architecture.md` §2.1.1 已知边界 |
...
| 已实现，真机待复验 | **扩展背包拖放回归修复（R-61/R-62）** | R-61：每帧 `clear_stale_projected_moving_locked` 曾在**空闲态**清掉原版 `TouchState+0x30`（`MOVING_CTRL`），打断原版触摸状态机、使 `0x18` 释放事件不再产生（真机实证：成功拖动后 `0x17`×48/`0x19`×700/`0x18`×0，拖动几件后失效）；改为仅会话非 idle 或触摸捕获时清、空闲态绝不触碰。R-62：`ownership::kLedgerCapacity` 32 过小（每个扩展物品占一个 module-owned 句柄 + `kInventoryOwned` 终态槽不复用，28 物品 + 4 终态即耗尽）→ `module materialize rejected ownership ledger exhausted` → `txn abort`（拖动可解析落点但无结果）；容量改 128 且 `handover_to_inventory` 交接即回收槽位 | 连续拖动可产生交换/跨袋移动结果；「扩展→原版」移动/装备后仍可继续拖动；`stale projected moving cleared` 与 `ownership ledger exhausted` 均不再出现 | 用户 2026-09-12；`rulebook.md` R-61/R-62；`verification-matrix.md` VM-45/VM-46；Host `test_ownership_ledger` |

> 规则：严格按下列顺序串行实施。每项完成后只部署该项，由用户在真机触摸验收；验收通过前不得开始下一项。

| 顺序 | 状态 | 待办项 | 验收标准 |
|---:|---|---|---|
| 2 | 探索中 | 背包物品跨原版/扩展背包拖拽移动 | 原版→扩展、扩展→原版均可移动物品，并遵循原版容量与堆叠规则。 |
...
| 进行中 | **craft mix 合成器交互** | ⛔ 原 P0「完成 api-reference 端点」收敛剩余项（2026-08-14 抽出）：合成链已逆向，需合成器交互验证（材料槽选中态/合成器上下文）。**2026-09-11 进展**：合成器（UIMix）扩展袋宿主已落地——页签/投影/状态门控（`extension_bag_mix.inc`）、R-56 按类别扣料（H-21 扩展桥接）、R-57 原版袋列高亮遮蔽；待真机验收 VM-22/VM-23。底层执行函数探索归 P4「合成执行」。遗留 ITEMPOOL_Free(0x108160) 符号登记消除产物入库失败的有界泄漏 | 合成器交互真机验证 + Service 层接线 | P0 端点收敛 + v0.5.21 |
