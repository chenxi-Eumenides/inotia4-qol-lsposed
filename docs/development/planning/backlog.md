...
| 已修复（2026-09-15） | **跨设备导入的 wh4 伴生文件不可用导致进档崩溃** | 2026-09-14 真机确证：`archive/20260914-221448_s1_324ea9787cde.qol_save` 导入 slot1 后进档 SIGSEGV（`CHAR_GetSkillPoint+0`，fault 0x328，GLThread）。根因：bundle 写回的 `save{slot}.dat.wh4-<hex>`（monster 改版个人仓库伴生文件，源设备原字节）不被本机接受 → 游戏 block3 校验器（libgame 无 section 隐藏段 `0x740000`/`0x7413b4`）返回 0 → `SAVE_LoadFile=0` → `SAVE_Load` 不调用 `SAVE_LoadCharacterAll` → 主控指针保持 `PLAYER_Initialize` 写入的 0 → `GAMESTATE_EnterPlay` 空指针崩溃。**移走该 wh4 后可正常进档**（`screen=world`、Lv105 角色与 3 人队伍完整）；wh4 缺失时游戏容忍（跳过仓库）。对照实验：仅移走遗留旧后缀 wh4 仍崩溃，移走全部（含导入档）才恢复。补充确证（2026-09-15）：wh4 缺失进档成功后，游戏**自行重建**同名同尺寸文件，且该重建版可再次正常进档。**拦截点已精确定位（运行时探针 probe15）**：`SAVE_LoadFile → block3 校验器 0x7413b4 → sub_1e64(0x741e64) 生成 …/save{N}.dat.wh4-<hex> → sub_3f14(0x743f14) 读文件并 `ENCRYPT_Process2(buf,len-3,mode=1,key)`（本机密钥）`；导入版的 `ENCRYPT_Process2` 返回 0（容器双校验和不匹配）、解密首 8 字节为乱码，而游戏重建版返回 1 且解密为 magic `WH4JRN01`。即 **wh4 与 `save{N}.dat` 用同一容器加密方案（payload+sum_cipher+seed+sum_plain，len+3 字节），密钥设备相关（`HubSave_GetKey`）**；模块导出只解密 `saveN.dat`、wh4 按原始密文搬运 → 换设备解不开。文件名后缀由保存内容派生，非文件名/大小问题。导出原样存 wh4 字节、导入按字节写回、不重加密（`docs/development/features/save-backup.md` §4/§5）。完整证据：`.tmp/crash-20260914/findings.md` §10（含 §10.8 拦截点日志、§10.9 修复方向）。 | **已实现**（2026-09-15）：导入时按本机密钥逐个校验 wh4 容器（游戏自身 `ENCRYPT_Process2(mode=1)` + `HubSave_GetKey`），**失败者不落盘**（游戏容忍缺失并会在下次成功读档后自建本机可用 wh4）；并由新增的 core 能力层 `qol::game_feature_state(kWarehouseCompanionFile)` 做变体能力门禁（无仓库机制的版本整步跳过）。真机验证：重新导入同一 bundle → `skip warehouse file save1.dat.wh4-0000019fb2e35d0e (not decryptable on this device)` ×2、`wrote=0 skipped=2` → `enter_slot(1)` 正常进 world，Lv105 角色与 3 人队伍完整、进程存活。变体识别由 `scripts/data/game_variant_table.py` 生成的校验值表（`data/native/game_variant_table.inc`，19 条）+ `game_variant()` / `game_feature_state()` 提供。文档：`docs/development/features/save-backup.md` §5 第 7 步、`docs/reference/game/save.md` §1.1/§3.1/§14。 | ① 若要**跨设备保留仓库内容**（而非丢弃后由游戏重建）：导出时解密 wh4、导入时用目标设备密钥重加密；② `preflight` 增加覆盖"block3/wh4 校验失败"的判决（当前只查槽结构，覆盖不到）；③ 确认游戏自建 wh4 是否完整保留了 block3 中的仓库内容。 | 跨设备导入后 `enter_slot` 不崩溃；wh4 有效样本仓库数据可读、无效样本结构化拒绝；真机验证通过 | 2026-09-14 真机调查 |
...
| 未开始 | op_* 参数校验补齐 + OP 能力隔离 | teleport(x/y/map 无范围)、learn_action(actionId/level 任意、无技能点校验)、sell(price 无约束)、move(x/y 无上限)、exp 截断 int32 校验策略不一致；OP native 实现（money/exp/statuspoint/teleport/sell 任意定价）仅靠「不挂路由」隔离，无权限机制（原 P0 H3 挂靠）。**2026-09-14 新实证**：世界连续传送测试中调用 teleport 复位 API 触发 `SPR_Draw → MAP_DrawLayer` SIGSEGV（世界加载/切图时序期 API 直切图），证据 `.tmp/world-teleport-insufficient/ten-cycle-crash.log`——API 侧切图缺世界就绪门禁，与本轮已修复的面板链路（`UIPopupMsg` 崩溃）无关。**2026-09-15 再次复现（monster v23）**：简单模式真机验证中连续跨图 teleport（21→34→40→20 四次连传）后约 0.6s 再次 SIGSEGV，栈为 `SPR_DrawFlip ← MAP_DrawPartial ← MAP_DrawBase`，fault addr `0xc8` 与 `0x234` 各一次；**同一构建、同一会话下不传送空跑 75 秒零崩溃**（健康轮询 25/25），因此高频跨图传送是本崩溃的触发条件，而非被包裹的伤害/属性函数。另注：`teleport` 按 **tile** 坐标、`move_to` 按**像素**坐标（1 tile = 16 px），两者单位不一致，误用会把角色送到图外 | 统一入口边界校验（坐标/槽位/枚举/价格≥0/int32 范围）；teleport/切图类 API 加世界就绪门禁（对照 P0「世界加载完成前保存崩溃防护」模式）；learn_action 先读技能点；OP 隔离：加全局开关（默认关闭）或移除，验证无 HTTP 路径可触发 | 审计 M6/M8 + 审计 H3 + 2026-09-14 连续传送测试 |
...
| 未开始 | **地图非敌人物件（出口指示符/宝箱/泉水）识别** | 用户 2026-08-09 实测：当前地图实际只有 2 个地图出口，units 输出 4 个「地图出口」（slot1-4，status=2）——**出口两侧各有指示符/标识物，名称同为「地图出口」（CHAR_GetName 来源）无法区分**；enemies 端点同样包含宝箱/泉水等非敌人物件（status==2 过滤语义）；宝箱/恢复泉水等交互点数据结构未探索 | 核对 status/type 语义或地图物件类型字段，区分真出口/指示符/交互物；enemies 过滤条件是否应排除非敌人（宝箱/泉水/出口）；探索交互点数据（宝箱/泉水结构 + 交互函数）。**2026-09-15 补充可直接复用的判定原语**：`CHAR_GetPartyIndex`（主角+2 队友，不含召唤物）、`CHAR_IsActivePlayerGroup`（主控本人+主控召唤物）、`CHAR_GetSummoner`（反查召唤者，递归可补齐队友的召唤物）；且 `C_TYPE(ch+0x09)` 语义已定案——取值即 `CHARSYSTEM_Produce` 的 type 参数：**0=玩家侧角色 1=怪物 2=NPC/装饰物**（`game_symbols.h` 原注释「0=英雄 1=佣兵」为错误口径，已纠正）。真机实测：练习01 的「箱子」也是 `C_TYPE==1` 且 `status==1`，故 type/status 都不足以单独排除宝箱，仍需交互点数据结构。见 `docs/development/features/simple-mode.md` §3 | 本会话测试 2026-08-09 + 本会话决策 + 2026-09-15 简单模式调查 |
...
| 状态 | 待办项 | 现状 / 卡点 | 需要的探索 / 实现 | 来源 |
|---|---|---|---|---|
| 未开始 | A1 伤害公式逆向 | 属性系数(0.66/0.5)×1.2、逐项取整、技能公式 Z%（参照 game-systems §6.1）。**2026-09-15 部分突破（落点已定位，公式内部常量未逆向）**：伤害结算唯一汇合点 = `CHAR_AddDamage`(0xea2d8)，签名 `(x0=攻击者, x1=受害者, w2=最终伤害, w3=标记(1=暴击/8=DOT), w4=0物理/非0法术)`；普攻/物理技能/法术/DOT/怪物动作/防御兜底共 **15 个静态调用点全部经此单点**（13 `bl` + `CHAR_AddDamageByDefaultCheck` 内 2 处尾调用 `b`），函数体第 3 条语句即唯一扣血点 `CHAR_AddLife(受害者, -伤害)`（0xea330）；旁路记录：`CHAR_SetAttribute(受害者,0x63,伤害)` 保存本次承伤、`CHAR_GetDistance>0x20` 时伤害×0x50/100 仅用于仇恨、`CHAR_IsActivePlayerGroup`/`CHAR_GetPartyIndex` 门控飘字。据此可直接做伤害倍率注入（见 `docs/development/features/simple-mode.md`） | 反汇编找 ×1.2/0.66 常量 + 取整逻辑（`CHAR_AttrAttackDamage`/`CHAR_AttrAttackDamageByType`/`CHAR_SuccessOnPhysical|MagicalAttack`/`CHAR_Get*Damage` 取值侧）；突破口：布甲 12% 魔攻加成 | 用户 2026-08-08 指定 P3 + 2026-09-15 定位汇合点 |
