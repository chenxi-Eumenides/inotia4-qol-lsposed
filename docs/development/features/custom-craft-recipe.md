...
**3 格隐式配方（§2.8）的验收状态（2026-09-15 起，monster v23 为主体）**：

- **VM-C19（文案）**：配方按钮显示「宝石升阶」**已由用户真机确认**；提示的「面板标题仍显示『宝石强化』」已修复（新增 `Scope::kUimixPanelTitle` 窗口，见 `custom-text.md` §4.1），**标题与页签的复验待用户执行**。
- **VM-C17 / VM-C18（配方命中与产物数值）**：**未验收**。合成器面板需要 NPC 交互上下文、API 不可开启，`/api/op/craft/mix-direct` 亦未实现，因此只能由用户实机合成确认；用户已确认合成所需的三种材料在背包中齐备（低级宝石 ×1 / 混沌武器强化卷轴 ×9 / 混沌防具强化卷轴 ×14）。
- 代码侧已通过：`custom_recipe_tests`（第 7 条的严格顺序 / 通配宝石槽 / 换位与缺格拒绝；`scaled_jewel_value` 的 ×1.2 向上取整与上限钳制）+ 全部 16 个 host test 目标 + debug 构建。
