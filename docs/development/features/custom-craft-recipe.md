...
| 7 | **任意宝石(28..32) + 混沌武器强化卷轴(20) + 混沌防具强化卷轴(25)** | **是** | **该宝石自身（同类别），数值 ×1.2（向上取整）** |

第 7 条的槽位是**通配宝石槽**（`kAnyJewelSlot`：任意 28..32）+ 两个固定卷轴类别，**顺序严格**（卷轴换位、缺格、第 1 格非宝石均不命中）。产物沿用源宝石的类别、随机等级（bits11-17）与属性类型（bits18-23），**只把数值位（bits0-10）按 ×1.2 向上取整**（`ceil(value × 1200 / 1000)`）并钳到上限 2047。类别依据：`ITEMDATABASE.json` 实证类别 20 → text_id 50「混沌武器强化卷轴」、类别 25 → 55「混沌防具强化卷轴」（全表满足「类别 = 名称 text_id − 30」）。
...
- **模块配方文案改写**：模块「宝石强化」条目（`Kind::kJewelTierUp`）的 label wordId 复用原版 35291，而**文本表里没有「宝石升阶」**、且 35291 在 UIMix 面板里被**三处**复用（配方按钮 / 面板标题 / 宝石强化页页签）。因此文案由 `feature/ui/module_text` 的内置条目在**三个对应窗口**分别处理：`kUimixRecipeButton`（hook `UIMix_ButtonRecipeDraw`）与 `kUimixPanelTitle`（hook `UIMix_Draw` 内的「当前配方名」）显示「宝石升阶」；`kUimixPageTab`（hook `UIMix_ButtonMenuListDraw`，页签）**显式不替换**、保持「宝石强化」。详见 `docs/development/features/custom-text.md` §4.1。
...
| VM-C17 | 混沌卷轴配方（3 格） | 宝石(28..32 任意档) + 混沌武器强化卷轴(20) + 混沌防具强化卷轴(25) 按序放入 → 确认后产物为**同类别**宝石，数值 = `ceil(源数值 × 1.2)`（钳上限 2047），随机等级与属性类型与源宝石一致；三件材料被消耗（宝石整颗、两种卷轴各 1 个） |
| VM-C18 | 顺序严格 | 卷轴换位（第 1 格非宝石）或缺格 → 弹「材料不足。无法进行合成。」且**不消耗任何材料** |
| VM-C19 | 配方文案三处同步 | 配方按钮与选中后的**面板标题**均显示「宝石升阶」；宝石强化页的**页签**保持原文「宝石强化」 |
...
**3 格隐式配方（§2.8）的验收状态**：**VM-C17 / VM-C18 / VM-C19 均已由用户实机确认通过**（2026-09-15，monster v23）：宝石 + 混沌武器强化卷轴 + 混沌防具强化卷轴 按序放入可合成、产物为同类别宝石且数值按 `ceil(源×1.2)` 缩放、顺序错误不消耗材料；配方按钮与选中后的面板标题均显示「宝石升阶」，宝石强化页页签保持原文「宝石强化」。用户未回传具体数值样本。

代码侧同批通过：`custom_recipe_tests`（第 7 条的严格顺序 / 通配宝石槽 / 换位与缺格拒绝；`scaled_jewel_value` 的 ×1.2 向上取整与上限钳制）+ 全部 16 个 host test 目标 + debug 构建。
...
32. **书名册索引不变式（缺陷 + 修复）**：注入 N 条 RECIPEBASE 记录把总记录数由 69 改成 69+N，而 `GetRecipeCount(0)` 返回的正是总记录数 → `AddRecipeBook`/`MakeRecipeList` 的 `base` 由 17 变成 17+N，产生三个可观察症状：① 既有解锁在传说装备页**整体错位 N 条**（已解锁的 bit0 原本显示记录 17「安格巴德的剑」，变成记录 19「龙之心脏」）；② `idx < 17+N` 的记录（D级武器合成套装的 17/18）解算出**负位索引**，而 `AddRecipeBook` 只查上界 → 对书名册缓冲前方越界读改写；③ 横幅取自 `RECIPEBASE[idx].b0-1`、与位图无关，且 `AddRecipeBook` 返回值被丢弃、`CHAR_ProcessRecipe` 恒返回 1 → 用户看到「提示解锁了 A，页面上却是 B / 没有 A」。
     ...
     **证据状态**：机制与修复依据为反汇编（§3.13）+ host 单测（`tests/test_custom_recipe.cpp` 第 6 项）。**真机复验通过（2026-09-16，monster v23）**：注入日志 `recipe table injected base=69 records=71 materials=191 book_bytes=7` —— 新加的 fail-closed 不变式 `book_bytes` 注入前后相等（7 == 7）放行，确认 `bit5` 已随注入记录写入；用户实测确认「融合器传说装备页的解锁项与横幅提示一致」，无错位、无负位记录，功能正常。bit5 置位对其它读点的影响同批确认无异常（设计上 group 5 的列表只扫书名册位图，注入记录对应的位 52/53 永不被解锁，故不出现新条目）。
33. **「合成套装在扩展背包可用、原版背包无效果」——游戏侧路径已排除，只剩两个候选（其一是本轮的索引偏移）**。
    ...
    **剩余候选**：① **本轮已修复的书名册索引偏移 +2**（§7.32）——解锁落到错误记录上，用户核对的配方仍处锁定态，表现就是「使用无效果」，这与「横幅说 A、页面是 B」是同一症状；② 模块 desc 按钮 wrapper `extension_desc_item_execute` 开头 `if (hook_index < 0 || hook == nullptr) return;` 这条**不回退原版、直接吞掉点击**的路径（当面板槽位指针与安装时不一致时命中；该函数其余分支都是 `hook->call_orig(button)` 正常回退原版）。
     **定案（2026-09-16 真机复验）**：候选 ① 确认为根因 —— 修复书名册索引偏移（§7.32）后，用户实测「横幅提示的配方名与融合器传说装备页的解锁项一致、功能正常」，说明「原版背包使用无效果」是**同一缺陷的表现**（解锁落到错误记录上，用户核对的配方仍处锁定态），无需另修使用路径。候选 ②（模块 desc wrapper 吞点击）在本次复验中未出现，保留为潜在风险记录：`extension_desc_item_execute` 开头 `if (hook_index < 0 || hook == nullptr) return;` 仍是一条不回退原版的分支，将来若出现「原版物品点击毫无反应」应先查它。
