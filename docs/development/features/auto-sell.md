...
1. **总开关**：启用/关闭整个自动出售。
2. **简要说明**：一行描述文本（总开关下方）。
3. **装备规则（3 条，值即开关，均为 `≤ 值-1`；三条之间为 AND，已开启项须同时命中；0=关不参与）**：
   - 品质：档位值（0=关；1..5 → 出售 `rarity ≤ 值-1`，1=白…5=紫）。
   - 强化耐久度：档位值（0=关；1..32 → 出售剩余强化次数 `I_ENCHANT` bits2-5 `≤ 值-1`；口径见 §3.1，用户 2026-09-15 裁决）。
...
- 强化等级/次数：`I_ENCHANT = 0x1A`（u16，`game_symbols.h:61`）；**bits6-10 = 当前强化/附魔等级**、bits11-15 = 附魔 ID（真机权威 `attribute-range-display.md:279`）。native 输出 `enchant_level`（`game_inventory_read.inc:18-22`），公开对象 `enchant:{id,level,effect}`（`api-reference.md:156`）。无读取 getter，只能读位域。
  - **规则口径（用户 2026-09-15 裁决，取代 2026-09-13 的 bits6-10 口径）**：强化筛选 = **剩余强化次数（`I_ENCHANT` bits2-5）**，界面名称「强化耐久度」。已强化次数（bits6-10）与已镶嵌数（`I_SOCKET` bits0-3）**不再参与规则比较，改由硬保护使用**（任一 >0 永不出售，见 §4.6 第 5 条）；人工写入这两段会造成游戏内不可能状态（实测：已强化=2 且强化 ID=0 时 `ITEMSYSTEM_EnchantItem` 返回 2、弹「与一般的强化卷轴不同」）。**无每件「强化上限」字段**，上限是常量（vanilla 30 / 改版 12），不用于比较。
...
- 装备（仅 `equip==true`）：**同类内 AND（用户 2026-09-16 裁决）**——已开启（值非 0）的装备规则必须**全部命中**才出售；值 0 的规则不参与判定；三条全关时不因装备规则出售：
  - 品质 `rarity`：0=关；`1..5` → 出售 `rarity ≤ 值-1`（1=白…5=紫）。
  - 强化耐久度 `enhance`：0=关；`1..32` → 出售 `enhance_remaining(I_ENCHANT bits2-5 剩余强化次数) ≤ 值-1`（界面名称「强化耐久度」，用户 2026-09-15 裁决口径）。
  - 镶嵌空位 `socket`：0=关；`1..16` → 出售 `socket_total(I_SOCKET bits4-7) ≤ 值-1`。
- **硬保护（规则判定之前执行，用户 2026-09-15 裁决）**：`enhance_level(I_ENCHANT bits6-10) > 0` 或 `socket_filled(I_SOCKET bits0-3) > 0` → 恒不出售（已强化或已镶嵌的装备无条件保护，不受任何配置影响；见 §4.6 第 5 条，实现于 `should_sell` 入口）。
...
| `enhance` | int `0..32` | `0` | 0=关；1..32 → 出售 `剩余强化次数（I_ENCHANT bits2-5）≤ 值-1`（界面名称「强化耐久度」） |
...
- 单次扫描（原版袋 `0..4` + 扩展 5 袋逐槽）：
  1. 取物品指针；空槽跳过。
  2. 分类：装备 / 宝石 / 特殊类型。
  3. 按 §4.3 规则判定（入口先过硬保护：已强化/已镶嵌恒不售；装备 `equip`+rarity/强化耐久度（剩余次数）/总孔数；宝石 `IsJewel`+category/属性百分位；特殊类型谓词）。
  4. 保护过滤（§4.6）。
  5. 处置（§4.5.1）。
...
4. 扩展背包事务中/锁定/投影中的物品。
5. 已强化或已镶嵌的装备（`I_ENCHANT` bits6-10 > 0 或 `I_SOCKET` bits0-3 > 0；用户 2026-09-15 裁决，无条件不受配置影响；实现于 `should_sell` 规则判定前）。
...
| R9 | 强化口径（2026-09-15 裁决：用剩余强化次数 bits2-5，界面名「强化耐久度」；已强化 bits6-10 与已镶嵌 bits0-3 改由硬保护排除；无每件上限字段） | 人工写 bits6-10 实测引发 `EnchantItem` 不可能状态 → 口径切换；host 单测覆盖边界 | 按 bits2-5 实现；真机回归 VM-AS4 |
...
- VM-AS4：装备强化规则只出售 `剩余强化次数（I_ENCHANT bits2-5）≤ 阈值` 装备；已强化装备（bits6-10>0）被硬保护排除、任何档位都不卖。
...
- VM-AS10：硬保护生效（已穿戴/任务/不可售/不可销毁/已强化/已镶嵌）。
- VM-AS11：开启时注册任务、扫描每 60 帧一次（日志帧号可核）；关闭时任务被删除（无回调）。
- VM-AS12：扩展袋物品处置物理槽与扩展账本一致。
- VM-AS13：空背包/满包/拖动中/存档界面/战斗中等边界无崩溃、无物品丢失。
- VM-AS14：装备三规则为 AND（已开启项须同时命中，关闭项不参与；三条全关不出售）；宝石同类内 OR、跨类命中任一即出售（用户 2026-09-16）。

## 8. 未决项

1. R1：`0x1261c4` 无 UI 可调用性；不可则改等价模块 sell 口径。
2. R9（2026-09-15 裁决）：强化筛选改用「剩余强化次数（`I_ENCHANT` bits2-5，界面名「强化耐久度」）」；已强化次数（bits6-10）与已镶嵌数（bits0-3）改由硬保护排除；每件「强化上限」不存在。残余：bits2-5 耐久度上限与初始化（新装备 remain 具体值）待真机核实。
...
- 规则组合（用户 2026-09-12，2026-09-16 修订）：**装备三规则之间取 AND**（已开启项须同时命中，0=关不参与；三条全关不参与判定）；**装备以外取 OR**——宝石两条规则同类内任一命中即出售，跨类（装备/宝石/特殊类型）命中任一即出售。
- 比较方向统一 `≤ 所选档`；孔位用总孔数；强化筛选口径：2026-09-13 曾用「当前总强化次数（bits6-10）」，用户 2026-09-15 改裁为「剩余强化次数（bits2-5，界面名『强化耐久度』）」——人工写 bits6-10 会造成游戏内不可能状态，已强化/已镶嵌改由硬保护无条件排除。
...
- **更新（2026-09-15 用户裁决，取代上条口径）**：筛选改用**剩余强化次数** bits2-5（界面名「强化耐久度」，即上述「疑为耐久/剩余次数」字段，经实测坐实）；已强化（bits6-10）与已镶嵌（`I_SOCKET` bits0-3）改由硬保护无条件排除，不再参与规则比较。证据：人工写 bits6-10=2 且强化 ID=0 → `ITEMSYSTEM_EnchantItem` 返回 2、弹「与一般的强化卷轴不同」（游戏内不可能状态）。
...
4. **强化口径与硬保护（2026-09-15）**：强化筛选口径由「总强化次数 bits6-10」改为「剩余强化次数 bits2-5（界面名『强化耐久度』）」；新增硬保护「已强化（bits6-10>0）或已镶嵌（`I_SOCKET` bits0-3>0）的装备永不出售」，无条件、不受配置影响，实现于 `should_sell` 规则判定前。构造接口 `/api/op/inventory/add` 相应只允许设置总孔数与剩余强化次数。
